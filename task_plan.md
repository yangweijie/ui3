...
- [x] 拖放 DnD（文件/文本/图片/URL）
  - 2026-07-30: `UI3_EVENT_DROP` + `ui3_host_inject_drop`；cocoa performDragOperation（文件/文本）/ win32 WM_DROPFILES（文件）/**x11 Xdnd v5**（2026-07-31：XdndAware property + Enter/Position/Drop/Finished + XdndSelection 检索 file:// URI 列表）；window `onDrop` + Automation `drop()` + 3 headless 测试
...
- [x] 原生无障碍树桥接（NSAccessibility / UIA / AT-SPI）
  - 2026-07-30: `ui3_host_set_a11y_text` 文本协议 + `flattenA11yTree`（PHP→tab-delimited text→C 解析）；C `ui3_a11y_node` 树 + `ui3_host_set_a11y_tree` deep copy + headless 序列化 + `ui3_host_last_a11y` 回读；cocoa NSAccessibility（Ui3View/Ui3A11yElement accessibilityChildren/Label/Description）/ **win32 UIA done**（2026-08-01：COM STA init + `IUIAutomationRegistrar` 注册 + 自定义 `IRawElementProviderSimple` vtbl + `WM_GETOBJECT` 窗口子类化；`UIA_ControlTypePropertyId`/`UIA_NativeWindowHandlePropertyId` 覆盖；5-bug fix + code review；16 种 role→ControlType 映射）/ x11 ATK stub（deferred — raw X11 无 GtkWidget）；5 headless 测试（text output/nested elements/root metadata/role mapping/non-accessible group）
- Status: P-Native P1 done（菜单栏/DnD/手势/对话框/通知/无障碍树全部落地；**win32 UIA done**（2026-08-01：5-bug fix + code review + 全文件审查确认无泄漏）；x11 ATK + x11 menu bar 仍 deferred（raw X11 无 GtkWidget；使用 `UI3_BACKEND=gtk4` 可获得完整支持））

### P-Native P2: 编辑/剪贴板/性能/后端覆盖
- [x] 富文本（bold/italic/underline/fontSize color 标签 prop → Cairo slant/weight + underline + fontSize 透传）
  - 2026-08-01: 新增 color prop 测试（red #ff0000 + bold+italic+fontSize 组合）；RichTextTest 7/7 pass
- [x] 剪贴板多格式（图片/RTF/fileURL）
  - 2026-07-30: `ui3_host_set/get_clipboard_image`（PNG bytes）、`set/get_clipboard_uris`（file:// URLs）、`set/get_clipboard_html`、`clipboard_formats`（UI3_CLIP_TEXT/IMAGE/FILES/HTML bitmask）；C ABI + common.c shared wrappers（headless 存 `last_clip_*` 字段）+ cocoa NSPasteboard（public.png / NSFilenamesPboardType / com.apple.html）+ **win32 全格式**（2026-07-31：`set_image`/`get_image` via `UI3_IMAGE_PNG` custom clipboard format, `set_uris`/`get_uris` via `UI3_URIS` custom format; HTML 原本已工作；**2026-08-01 CF_TEXT→CF_UNICODETEXT 修复**）+ x11 GTK3 多格式（2026-07-31 修复：get_image 悬空指针→malloc 复制；set_html NULL 回调→`gtk_clipboard_set_with_data` + get-callback；set_image stub→完整实现；formats 仅 TEXT→`gtk_clipboard_wait_for_targets` 查真实 atoms）；Canvas.php 7 个方法（setImage/getImage/setUris/getUris/setHtml/getHtml/formats）；5 headless 测试全部通过；Bug fix：`nativeClipboard()` 原本 `!$this->isHeadless()` 阻塞 headless 调用→改为 `$this->host !== null`；`ui3_host_set_clipboard_text` 平台函数未存 `last_clip_text`→三平台加 headless 存储
- [x] compositor / GPU 层 / 局部重绘
- [x] scroll 水平滚动 + 水平滚动条拖拽（2026-08-04：对标 native SDK Issue #137）
  - `Layout` 加 `$scrollContentW`/`scrollContentWidth()`/`$overridesX`（`case 'scroll'` 逐 child 测自然宽累计内容宽 + offX）；`Canvas` 加 `$scrollOverridesX`/`$scrollElasticX` + `scrollByX`/`scrollToX`/`effectiveScrollOffsetX`/`scrollOffsetX`；水平滚动条底缘轨道（`scrollbarGeomX`/`hitScrollbarX`/`paintScrollbarX`/`drawScrollbarX`，thumbW=max(16, vw/contentW×trackW)）；`tryBeginScrollbarDrag` 垂直优先再水平 + drag struct `axis` 键；←/→ 方向键；**四平台水平滚轮**经 `UI3_EVENT_WHEEL` 的 `text` 字段传水平 delta（零 ABI 变更）：win32 `WM_MOUSEHWHEEL` / x11 Button6/7 / cocoa `scrollingDeltaX` / gtk4 Button 6/7（dy=0）；Canvas `onEvent` 解析 `text` → `scrollByX`（|dx|≥0.5）；ScrollInteractionTest 12 passed / 27 assertions
- [x] Wayland / GTK4
  - 2026-07-31: `ext/gtk4.c` 全新 GTK4 后端（768 行）；GTK4 原生支持 X11 + Wayland 双协议，单个后端同时覆盖 Wayland parity 和修复 x11.c 遗留缺陷（菜单栏/无障碍/剪贴板多格式/原生对话框）。编译链路：`UI3_BACKEND=gtk4 bash ext/build.sh`（pkg-config gtk4），CI 新增 `test-gtk4` job（ubuntu-latest + libgtk-4-dev + 冒烟测试）。**2026-08-01 CI job 加固：移除 continue-on-error + 冒烟过滤扩展 `Clipboard|RichText|Compositor|WidgetsParity` + stderr 可见 + `UI3_BACKEND=gtk4` env var + `--no-coverage`**。已删死代码 `ext/null.c`（41 行陈旧 `ui3_backend_ops` API，零引用）。
- [ ] 移动端 / 真 WebView
...
- Status: 剪贴板多格式 done（**三平台全格式 done**：cocoa 全格式、**win32** 全格式（CF_UNICODETEXT 修复 2026-08-01）、x11 GTK3 全格式）；富文本 done（bold/italic/underline/fontSize color 标签 prop → Cairo + 7 回归测试）；compositor done（backing surface + dirty rect + partial blit + 16 unit tests）；IME 完整性 done（Canvas pixel-level offscreenPixels 测试 + App 端到端路由 + compositor blit 修复）；拼写检查 done（Spellcheck 纯 PHP 引擎 + 系统词典静态共享 + Canvas/Reference 红色波浪线 + 右键建议/Add to dictionary/Ignore/替换，SpellcheckTest 11 + SpellTest 3 + CanvasSpellTest 5 全过）；**x11 DnD done**（Xdnd v5）；**GTK4/Wayland done**（ext/gtk4.c + build.sh UI3_BACKEND 分支 + CI job 2026-08-01 加固；移动端/Webview 排除已记录理由）
...
另开 **P-Native（Native SDK Parity）** 跟踪块，对标 OS 原生 GUI SDK 补齐系统集成深度（P0 修饰键/Cmd 捕获、多窗口/窗口管理、菜单栏/托盘/DnD/手势/对话框/通知/无障碍树均 done（**win32 UIA 5-bug fix + 全文件审查** 2026-08-01：COM STA + `IUIAutomationRegistrar` + `IRawElementProviderSimple` vtbl + `WM_GETOBJECT` 子类化 + `UIA_ControlTypePropertyId`/`UIA_NativeWindowHandlePropertyId` + 错误路径无泄漏，16 种 role→ControlType 映射）；P2 剪贴板多格式 done（**三平台全格式**：cocoa / **win32** CF_UNICODETEXT / x11 GTK3）；富文本 done（+ color prop 测试 2026-08-01）；compositor done；IME 完整性 done；拼写检查 done（19 测试）；**GTK4/Wayland done**（ext/gtk4.c + build.sh UI3_BACKEND 分支 + CI job 2026-08-01 加固：移除 continue-on-error + 冒烟过滤扩展）；**x11 DnD done**（Xdnd v5）。剩余 deferred（已记录原因）：x11 ATK + x11 menu bar（raw X11 无 GtkWidget；`UI3_BACKEND=gtk4` 可获完整支持）。
