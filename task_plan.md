...
- [x] 剪贴板多格式（图片/RTF/fileURL）
  - 2026-07-30: `ui3_host_set/get_clipboard_image`（PNG bytes）、`set/get_clipboard_uris`（file:// URLs）、`set/get_clipboard_html`、`clipboard_formats`（UI3_CLIP_TEXT/IMAGE/FILES/HTML bitmask）；C ABI + common.c shared wrappers（headless 存 `last_clip_*` 字段）+ cocoa NSPasteboard（public.png / NSFilenamesPboardType / com.apple.html）+ win32 HTML Format（image/uris deferred）+ x11 GTK3 多格式（2026-07-31 修复：get_image 悬空指针、set_html NULL 回调→改用 `gtk_clipboard_set_with_data` + get-callback + cleanup-callback；set_image 从 stub 改为完整实现；formats 从仅 TEXT 改为 `gtk_clipboard_wait_for_targets` 查真实 target atoms 映射）；Canvas.php 7 个方法（setImage/getImage/setUris/getUris/setHtml/getHtml/formats）；5 headless 测试全部通过；Bug fix：`nativeClipboard()` 原本 `!$this->isHeadless()` 阻塞 headless 调用→改为 `$this->host !== null`；`ui3_host_set_clipboard_text` 平台函数未存 `last_clip_text`→三平台加 headless 存储
